 ...
 * @author m.yoshitake
 */
class Model_Omophoto_Ranking_Total extends Model_Base{
	...
	protected static $_table_name = 'omophoto_ranking_totals';
	 ...
	 * @return array
	 */
	public static function list_data($need_num, $target_at=null, $max_id=null, $user_id=null, $official_flg=null, $no_cache=null){
		if(empty($target_at)){
		...
		$query->as_object('Model_Omophoto_Ranking_Total');
		...
		if($no_cache == 1){
			...
			$cache_key = "db." .self::$_table_name .".n{$need_num}_t" .substr(mb_ereg_replace('[^0-9]', '', $target_at),0,8) ."_m{$max_id}_u{$user_id}_o{$official_flg}";
	 ...
	 * @return array
	 */
	public static function omophoto_ranking($need_num, $target_at=null, $max_id=null, $user_id=null, $official_flg=null, $no_cache=null){
		$respons['omophotos'] = self::list_data($need_num, $target_at, $max_id, $user_id, $official_flg, $no_cache);
	...
	public static function get_max_target_at(){
		$max_target_at = DB::select(DB::expr('max(target_at) as target_at'))
						->from(static::table())
						->execute();
		if(!$max_target_at[0]["target_at"]){
