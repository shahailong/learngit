 ...
 * @author a-kido
 */
class Model_Specialcollection extends Model_Base
{
	 ...
	 */
	public static function filter_response_records($excutor_id, array $specialcollection_models){
		if(!$specialcollection_models){
			return $specialcollection_models;
		}

		$return_ary = array();

		foreach($specialcollection_models as $specialcollection_model){
			if(!($specialcollection_model instanceof Model_Specialcollection)){
				throw new Exception_Logic('Illegal argument.');
			}

			if((string)$excutor_id != (string)$specialcollection_model->id){
				// プライバシーに関わる情報をマスクする
				$specialcollection_model->mask_unvisible_info();
			}
			$return_ary[] = $specialcollection_model;
		}
		return $return_ary;
	}
	...
	public static function list_data($need_num, $max_id = null, $user_id=null, $no_cache=null){
		...
		$query->as_object('Model_Specialcollection');
